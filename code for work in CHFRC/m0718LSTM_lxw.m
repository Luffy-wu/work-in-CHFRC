...
% input variables，设置各层的权重个数，输入层、输出层、隐藏层设一个
alpha      = 0.1;
input_dim  = 2;
hidden_dim = 32;
output_dim = 1;
...
out_para = 2 * rand(hidden_dim, output_dim) - 1;  %定义最后计算输出结果的权重
...
%train 
iter = 19999; % training iterations   %原是99999，嫌太慢，我改成了19999
for j = 1:iter
    ...
    for position = 0:binary_dim-1    %··对每一个输入数a,b，用X记录某一位数值（从高位往低位），如a=00010001,b=00001000，则X为[1 0]，并计算每一位相加的预测值pred_out，以及误差 output_error 
        %··最后一步，d(binary_dim - position) = round(pred_out);是pred_out取整，赋给预测值d的相应那一位
        %··这个for结束后，in_gate，forget_gate，out_gate，
        %··g_gate，C_t，H_t，各有8个数列，存放于I 、 F 、 O、 G 、C、H，
        %··output_deltas~output_error也是，用于这个for end下面的更新
        % X ------> input, size: 1 x input_dim  
        X = [a(binary_dim - position)-'0' b(binary_dim - position)-'0']; %0803，从a的最高一位开始
